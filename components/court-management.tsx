...
function CourtsSection() {
  ...
  const handleToggle = async (court: Court) => {
    ...
    const data = await res.json()
    if (data.success) {
      setCourts((prev) => prev.map((c) => (c.id === court.id ? data.data : c)))
    }
    setTogglingId(null)
  }
  ...
  return (
    <Card className="p-6 border border-border/50">
      ...
      <p className="text-sm text-muted-foreground mb-5">
        Toggle courts active/inactive to adjust booking capacity. Only active courts count toward availability.
      </p>
          ...
          {courts.map((court) => (
            ...
            >
              <div className="flex items-center gap-3">
                <span className={`w-2.5 h-2.5 rounded-full ${court.is_active ? 'bg-emerald-500' : 'bg-muted-foreground'}`} />
                <span className="font-medium text-sm text-foreground">{court.name}</span>
                <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${
                  court.is_active ? 'bg-emerald-500/15 text-emerald-600' : 'bg-muted text-muted-foreground'
                }`}>
...
function DateOverridesSection() {
  ...
  const handleDelete = async (id: number) => {
    setDeletingId(id)
    const res = await fetch(`/api/admin/date-overrides/${id}`, { method: 'DELETE' })
    const data = await res.json()
    if (data.success) setOverrides((prev) => prev.filter((o) => o.id !== id))
    setDeletingId(null)
  }
